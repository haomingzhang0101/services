package com.haoming.house.user.config;
...
public class RestAutoConfig {
    ...
    public static class RestTemplateConfig{
        @Bean
        @LoadBalanced //二次定制化RestTemplate这个bean，加入loadbalance拦截器进行ip:port的替换
        RestTemplate lbRestTemplate(HttpClient httpClient) { //spring自动找相应的bean
        ...
        @Bean
        RestTemplate directRestTemplate(HttpClient httpClient) { //spring自动找相应的bean
            RestTemplate template = new RestTemplate(new HttpComponentsClientHttpRequestFactory(httpClient));
            template.getMessageConverters().add(0, new StringHttpMessageConverter(Charset.forName("utf-8")));
            template.getMessageConverters().add(1, new FastJsonHttpMessageConvert5());
            return template;
